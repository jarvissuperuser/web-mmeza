...
/*    {path: '/home', view: {element: '<explore-page></explore-page>'},visible: true},
    ...
    {path: '/landing', view: {element: '<landing-page></landing-page>'}, visible:true},
...
export const resolvePath = () => {
    ...
    const routePathHash = getRoutePath();
    const route = routePathHash[1];
    if (!!route && route.indexOf('/') === 0){
    if (!!route && routes.map(r => r.path).includes(route)) {
        navigate(route);
        return true;
    }
    ...
    window[singleton.resolveSingleton]();
    //
}
